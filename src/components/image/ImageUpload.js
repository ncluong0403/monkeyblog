...
const ImageUpload = (props) => {
  ...
  return (
    <label
      className={`cursor-pointer flex items-center justify-center border-2 border-dashed w-full min-h-[200px] rounded-lg ${className} relative overflow-hidden group`}
    >
      <input
        type="file"
        name={name}
        ...
        <Fragment>
          <img
            ...
            className={`object-cover w-full h-full p-1 ${
              roundedFull ? roundedFull : "rounded-lg"
            }`}
            alt=""
          />
          <button
